...
def read() -> list[Contact]:
    ...
    for result in results:
        new_contact = Contact(result["name"], result["contact_number"], result["position"], result["date_created"], result["date_updated"])
...
def update(position: int, name: str, contact_number: str) -> None:
    if name is not None and contact_number is not None:
        db.update({"name": name, "contact_number": contact_number}, ContactQuery.position == position)
